...
## Seeded expansions beyond A1 (A1a–A4a, B1–B3b) and their quirks

The catalog seeds every A-series set through A4a and the whole released B-series (B1 through B3b); each is one JSON file under `data/`, fetched by the pipeline below and registered in `catalog.ts`. Per-set quirks, in the A1-quirks style:
...
- **flibustier-sourced rarity (B1a Crimson Blaze onward).** dotgg has **not backfilled rarity** for the sets from B1a on — every card's dotgg `rarity` is `null` (one B1a card excepted) — but is otherwise complete (HP, attacks, text, images, illustrator, shop). Rarity is therefore sourced from the **flibustier/pokemon-tcg-pocket-database** index (`dist/cards.json` over `raw.githubusercontent.com`), consulted by the pipeline only for cards whose dotgg rarity is missing. Card *body* provenance stays dotgg (`source.provider = "dotgg.gg"`, `source.slug` the dotgg slug); rarity provenance for these sets is the flibustier index, recorded here. It carries **distinct `SR`/`SAR` codes**, so the `☆☆` tier is split correctly into Super Rare and Special Art Rare — the SAR alternate-art subset is *not* collapsed (an earlier TCGdex attempt reported both as one `Two Star` tier and could not tell them apart). The index was validated card-for-card against every dotgg-authoritative seeded set (A1–A4a + B1, 1,911/1,912 rarities; the single difference, `A2-022` Shaymin, is a dotgg error the index gets right). The pipeline enforces the registry count, so a rarity gap never changes the card total.
- **Stadium trainer subtype (B2, Fantastical Parade).** First set with the `Stadium` subtype (e.g. `B2-153` Training Area); `mapTrainerSubtype` in `fetch-set-data.mjs` already handled it. B2a and later B-sets also carry Stadiums.
- **B3a `Ancient`/`Future` unsourced.** Paradox Drive (B3a) debuts the `Ancient`/`Future` classifications in-game, but neither dotgg nor the flibustier index exposes them, so `pokemon.classification` stays `null` for every B3a card — the same "modeled but unsourced" situation as `isBaby`. Revisit if a source ever carries it. B4 (Ruler of the Skies) is unreleased.
...
**`dotgg.gg`** card database is the primary source (`source.provider = "dotgg.gg"`, `source.slug` like `a1-1-bulbasaur`); **`pocket.limitlesstcg.com`** supplies flavor text; the **flibustier/pokemon-tcg-pocket-database** index on **`raw.githubusercontent.com`** is a rarity fallback for the newer sets dotgg has not backfilled rarity for (B1a on). All endpoints and the field mapping below were **confirmed against live responses (2026-07)**; the fetcher was re-derived from real records and reproduces the seeded A1 file byte-for-byte (flavor aside). A future wire-format change stays isolated to `normalizeDotggCard` / `parseLimitlessFlavor` / `mapFlibustierRarity` in `fetch-set-data.mjs`, since everything downstream is tested against the `sourceCardSchema` contract.
...
- **flibustier (rarity fallback):** `GET https://raw.githubusercontent.com/flibustier/pokemon-tcg-pocket-database/main/dist/cards.json` — one bulk JSON array (`{ set, number, rarity, name, … }` per card across every set). Downloaded once per fetch and filtered to the set, keyed by `number`. The fetcher consults it **only** for cards whose dotgg rarity is missing (`KNOWN_RARITY_CODES` gate in `fetch-set-data.mjs`), so a set dotgg fully describes never touches it. Unlike flavor, rarity is **required**: a host block aborts the fetch (never ships a card without a rarity), while a card the index omits (or gives an unrecognized code) is left unmapped so `cardSchema` names it. Its short codes → app tuples via `mapFlibustierRarity` (`IM`→`IR`, `UR`→`CR`; distinct `SR`/`SAR`). Validated 1,911/1,912 against the dotgg-authoritative sets (A1–A4a + B1).
