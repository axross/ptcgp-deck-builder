...
| Card-catalog data or catalog/deck access functions | Rendered output, content fidelity |
| Deck persistence (localStorage store modules and their schemas) | Saved decks, content fidelity |
...
- MUST verify any non-default content state the surface supports (e.g., an empty deck list, a deck at the size limit, or corrupt/absent localStorage data) in addition to the default state.
...
This project relies on end-to-end tests as the primary verification mechanism. E2E tests drive the real application in a real browser and cover the full pipeline from the card catalog and browser-persisted decks through to the rendered output.
