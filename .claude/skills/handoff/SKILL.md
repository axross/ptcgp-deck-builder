...
when_to_use: Invoke when the human wants to stop working here and let another session pick the work up — "hand this off", "wrap this up for another session", "package this up so we can continue later". It suspends at the nearest safe boundary and captures state. Do not invoke to actually finish, commit, or push the work, or when there is no in-progress work to suspend. Takes no arguments.
