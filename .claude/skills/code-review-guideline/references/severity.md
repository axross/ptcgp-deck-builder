...
| Unsanitized user input flowing into a storage lookup, request handler, or content-rendering pipeline | Critical |
...
| Missing `await` on a Promise returned from a storage or network call | Critical |
