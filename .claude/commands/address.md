...
- **A human decision — a Phase 1 Must-ask, plan approval, an ambiguous finding, or a conflict judgment call:** ask it through the dedicated question UI and use the answer inline (see [Asking the Human](#asking-the-human)). When no synchronous answer can be obtained (a headless run has no human, or the question call errors/closes), or when a machine event is stuck, post your state, **end the turn**, and wait at zero cost; the human resumes by sending **`/address continue`** in this same session — and a bare answer they type then is reconciled with the still-open question, not treated as a new request.

- MUST poll autonomously ONLY for machine events (CI, the review workflow); never keep a session alive polling for a human.
- MUST resolve human decisions through the question UI inline when it is available (see [Asking the Human](#asking-the-human)); end the turn for `/address continue` when no synchronous answer can be obtained or a machine event is stuck, and never schedule a wake-up to re-check for human input.
...
- A closed or errored question call is **not proof the surface can't render the UI**. On a remote/cloud session the permission stream can close *transiently* — no client was synchronously attached at that instant — even though the human is reachable and answers on a later turn (the same async assumption behind `send_later` and `/address continue`). Only a genuinely headless run — a routine/API job with no human to answer — is *permanently* unable to render it. A single stream-close error (in Claude Code, `Tool permission stream closed before response received`) cannot by itself tell transient-async from permanent-headless, so **do not diagnose the surface from it**: treat the ask as unanswered-for-now and fall back, without asserting the UI is unavailable.
- MUST fall back to a marked GitHub comment that @mentions `@axross`, then **end the turn**, whenever the question call errors or returns no synchronous answer. The fallback's options MUST be **identical and identically numbered** to the `AskUserQuestion` call, so an answer arriving through any channel — the UI on a later turn, or a bare token typed in chat — maps to exactly one choice; drifted numbering would silently select the wrong branch. Phrase the comment as *couldn't get a synchronous answer; decision recorded on the issue* — **not** *the UI is unavailable on this surface*. The comment is a durable breadcrumb `/address continue` resumes from, so the run never stalls.
- MUST treat the question as **still open** until it is actually answered. On the next turn — a bare answer token (an option number, its label, or free-form "Other" text) arriving as a plain message, or a `/address continue` — attribute that answer to the open question and reconcile it against the recorded options; do not re-ask or restart. Because the fallback mirrors the UI's option numbering, the same token resolves to the same choice through either channel.
- MUST keep the pinned status comment current with any open question — including its exact options and numbering — so a reclaimed session recovers it regardless of which channel asked.
...
- For `continue`, re-read the target's current state — the open pull request, its CI status, the independent review's comments, unresolved threads, and your pinned status comment — before acting, and resume the single pending step rather than restarting. If the pinned status comment records an open question and the resume carries a bare answer token, attribute it to that question and reconcile it against the recorded options rather than re-asking (see [Asking the Human](#asking-the-human)).
